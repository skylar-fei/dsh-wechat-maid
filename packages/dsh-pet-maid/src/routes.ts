 ...
 * same-origin JSON endpoints (`/api/pet-maid/*`) and loads the maid atlas
 * from `/pet/maid/*`. The `/plugins/` endpoint only serves client bundles
 ...
 * @module @deepseek-ai/dsh-pet-maid/routes
 */
...
export const PET_API_PREFIX = '/api/pet-maid'

/** Browser-facing base path of the pet asset routes. */
export const PET_ASSET_PREFIX = '/pet/maid'

/** Relative (to package root) asset files exposed under the prefix. */
const ASSET_FILES = [
  { name: 'spritesheet.png', mime: 'image/png' },
...
export function makePetRoutes(deps: { service: PetService; packageRoot: string }): WebRoute[] {
  ...
  const apiRoutes: WebRoute[] = [
    ...
    postRoute(`${PET_API_PREFIX}/set-auto-coding`, (body) => {
      const enabled = body.enabled
      if (typeof enabled !== 'boolean') return Promise.reject(new Error('invalid-enabled'))
      return service.setAutoCoding(enabled)
    }),
  ]
  ...
  const assetRoutes: WebRoute[] = ASSET_FILES.map((file): WebRoute => ({
    ...
    handler: (req: IncomingMessage, res: ServerResponse): Promise<void> | void => {
      ...
      return readFile(join(packageRoot, 'assets', 'maid', file.name)).then((body) => {
        res.writeHead(200, {
