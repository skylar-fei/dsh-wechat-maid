 ...
 * @module @deepseek-ai/dsh-pet-maid/affinity
 */
...
export const AFFINITY_RANKS = [
  { min: 0, name: '初见', emoji: '*' },
  { min: 25, name: '熟识', emoji: '**' },
  { min: 50, name: '亲密', emoji: '***' },
  { min: 80, name: '心灵之约', emoji: '****' },
] as const
...
export function applyInteraction(
  ...
  if (kind === 'pet') {
    if (state.lastPetAt !== 0 && nowMs - state.lastPetAt < config.petCooldownMs) {
      return { affinity: state, delta: 0, reaction: '摸过头啦，让{name}歇口气～', accepted: false }
    }
