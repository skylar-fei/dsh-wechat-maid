/**
 * maid spritesheet geometry and animation tracks.
 *
 * The atlas follows the Codex/hatch-pet contract: 8 columns × 10 rows of
 * 384×416 cells (3072×4160 total), rows in this order:
 *   0 idle, 1 running-right, 2 running-left, 3 waving, 4 jumping,
 *   5 failed, 6 waiting, 7 running, 8 review, 9 sleeping,
 *   10 petted, 11 poked, 12 angry, 13 flail
 *
 * Frame counts and per-frame durations are per-track definitions below; the
 * maid atlas is generated programmatically from a single Q版角色底图, so calibrate
 ...
 * @module @deepseek-ai/dsh-pet-maid/client/spritesheet
 */
...
export const FRAME_HEIGHT = 416
/** Columns per row (max frames per track). */
export const FRAME_COLUMNS = 8
/** Rows in the atlas (one per animation track). */
...
/**
 * Track definitions for the maid. Durations are tuned for a soft,
...
export const TRACKS: Record<PetAnimation, TrackDef> = {
  idle: { frames: [0, 1, 2, 3, 4, 5], durations: [350, 350, 400, 350, 350, 400], loop: true },
  'running-right': { frames: [0, 1, 2, 3], durations: [200, 200, 200, 200], loop: true },
  'running-left': { frames: [0, 1, 2, 3], durations: [200, 200, 200, 200], loop: true },
  waving: { frames: [0, 1, 2, 3, 4, 5], durations: [200, 200, 250, 200, 200, 250], loop: true },
  jumping: { frames: [0, 1, 2, 3], durations: [150, 120, 300, 250], loop: false, fallback: 'idle' },
  failed: { frames: [0, 1, 2, 3], durations: [450, 450, 550, 600], loop: false, fallback: 'idle' },
  waiting: { frames: [0, 1, 2, 3, 4, 5], durations: [400, 400, 450, 400, 400, 450], loop: true },
  running: { frames: [0, 1, 2, 3], durations: [200, 200, 200, 200], loop: true },
  review: { frames: [0, 1, 2, 3, 4, 5], durations: [450, 450, 500, 450, 450, 500], loop: true },
  sleeping: { frames: [0, 1, 2, 3, 4, 5], durations: [650, 650, 750, 650, 650, 750], loop: true },
  petted: { frames: [0, 1, 2, 3, 4, 5], durations: [250, 250, 300, 250, 250, 300], loop: true },
  poked: { frames: [0, 1, 2, 3], durations: [160, 160, 160, 160], loop: true },
  angry: { frames: [0, 1, 2, 3, 4, 5], durations: [220, 220, 260, 220, 220, 260], loop: true },
  flail: { frames: [0, 1, 2, 3], durations: [140, 140, 140, 140], loop: true },
}
 ...
 * @param image - the fully decoded spritesheet (natural size 3072×4160).
 * @returns per-row frame counts, length FRAME_ROWS.
 */
export function detectFrameCounts(image: HTMLImageElement): number[] {
  ...
  if (ctx === null) return Array.from({ length: FRAME_ROWS }, () => FRAME_COLUMNS)
